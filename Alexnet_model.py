...
def conv(x, filter_height, filter_width, num_filters, stride_y, stride_x, name,
         padding='SAME', groups=1, not_pretrain=False):
    ...
    with tf.variable_scope(name) as scope:
        ...
        weights = tf.get_variable('weights', shape=[filter_height,
                                                    ...
                                                    num_filters], initializer=tf.contrib.layers.xavier_initializer(),trainable=not_pretrain)
        tf.add_to_collection('params', weights)
        biases = tf.get_variable('biases', shape=[num_filters], trainable=not_pretrain)
...
def batch_norm(inputsTensor, is_training, name):
    """
    Batchnorm
    """
    _BATCH_NORM_DECAY = 0.99
    _BATCH_NORM_EPSILON = 1e-12
    return tf.layers.batch_normalization(inputs=inputsTensor, training=is_training, name=name,
...
class Model(object):
    ...
    def __init__(self, x, out_dim, is_training, not_pretrain):
    ...
    def load_with_skip(self, sess, data_path, skip_layer):
        """
        ...
        :return:
        """
        self.data_dict = np.load(data_path, encoding='latin1').item()

        for key in self.data_dict:
            if key not in skip_layer:
                with tf.variable_scope(key, reuse=True):
                    for subkey, data in zip(('weights', 'biases'), self.data_dict[key]):
